...
int MaxFunc(int cost, int value) {
    return 4 * value - cost;
}

void DeleteBad(unordered_set<int> &free, int cur_k) {
    vector<int> del;
    for (auto cur_g: free) {
...
void Solve() {
    ...
    while (!free.empty()) {
        ...
        for (auto cur_g : free) {
            ...
            if (MaxFunc(sz(pts), sz(group_pts[cur_g])) > ma) {
                ma = MaxFunc(sz(pts), sz(group_pts[cur_g]));
                g = cur_g;
                cost = sz(pts);
